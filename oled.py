...
def show_info(cpu,memory):
    oled.fill(0)
    oled.show()
    oled.text("cpu",10,5,1)  #显示字符串
    oled.text(f'{cpu}%', 70,5,1)
    oled.text("memory",10,35,1)  #显示字符串
    oled.text(f'{memory}%', 70,35,1)
    oled.show()  #执行显示
...
#     oled.show()  #执行显示
    while True:
        show_info(random.randint(1,100),random.randint(1,100))
        time.sleep_ms(500)
